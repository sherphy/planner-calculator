...
export default function ItemDetails({ itemId , points}) {
  ...
  useEffect(() => {
    async function fetchItemData() {
      try { 
        const response = await axios.get(`https://maplestory.io/api/${region}/${version}/item/${itemId}/name`);
  ...
  return (
    <div>
      {itemData ? (
        <div className="item-details" style={{display: "flex", alignItems: "center", margin:"0.5em 0"}}>
          <img src={`https://maplestory.io/api/${region}/${version}/item/${itemId}/icon`} alt={itemData.name} />
